...
func _ready() -> void:
	...
	publish_divider_label.add_theme_font_size_override(
		"font_size", theme.get_font_size("main_size", "EditorFonts")
	)
...
func _on_toggle_visibility(prime_id: String, name: String, current_is_public: bool) -> void:
	...
	var res: Dictionary = await exporter.set_prime_visibility(self, _token, prime_id, new_is_public)
	...
	await logs.append_log(
		"Toggled visibility for [b]%s[/b] → %s" % [name, "public" if new_is_public else "hidden"]
	)
...
func _on_update_prime_meta(
	prime_id: String, prev_name: String, name: String, description: String
) -> void:
	if _token.is_empty():
		await logs.append_log(
			"[color=orange]Session expired. Please sign in again.[/color]", "orange"
	...
	var res := await exporter.update_prime_meta(self, _token, prime_id, name, description)

	if not res.get("success", false):
		await logs.append_log(
			(
				"[color=red]Update failed for[/color] [b]%s[/b]: %s"
				% [prev_name, String(res.get("error", "unknown"))]
			),
			"red"
		)
		return
...
func _on_flag_appeal_submitted(prime_id: String, flag_id: int, message: String) -> void:
	...
	await logs.append_log(
		(
			"[color=green]Appeal submitted for flag[/color] [b]%d[/b] on [b]%s[/b]."
			% [flag_id, prime_id]
		)
	)
...
func _perform_prime_delete(prime_id: String, name: String) -> void:
	...
	if not res.get("success", false):
		var err := String(res.get("error", "unknown"))
		await logs.append_log(
			"[color=red]Failed to delete[/color] [b]%s[/b]: %s" % [name, err], "red"
		)
		return
...
func _on_publish(name: String, description: String, hide_from_feed: bool) -> void:
	...
	var result := await pack_and_upload(self, _token, is_public, name, description)
...
func pack_and_upload(
	host: Node, token: String, is_public: bool, name: String, description: String
) -> Dictionary:
